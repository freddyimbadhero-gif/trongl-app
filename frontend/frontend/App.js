...
// Adresa vašeho backendu (v produkci reálná URL, pro dev např. http://10.0.2.2:8000 na Android emulátoru)
...
export default function App() {
  ...
  // 1. Získání aktuální polohy uživatele
  useEffect(() => {
    (async () => {
  ...
  // 2. Nacitání tras z TRONGL Backend API
  const fetchRoutes = async () => {
    if (!location) return;
    setLoading(true);

    try {
      // Testovací cíl cca 1km od uživatele
      ...
      if (data.routes.length > 0) setSelectedRoute(data.routes[0]); // Vybere nejbezpečnější
    } catch (error) {
  ...
  // 3. Nahlášení incidentu z mobilu
  ...
  return (
    <View style={styles.container}>
      ...
      {/* SPODNÍ PANEL S MOŽNOSTMI TRASY */}
      {selectedRoute && (
